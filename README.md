...
Charge transfer occurs from $\text{Fe}$ to $\text{Au}$, thereby favoring the $\text{Fe}_y$ sites for nucleophilic $\text{CH}_4 + \text{H}_2\text{O}_2$ attacks. Our results show that the whole $\text{CH}_4 + \text{H}_2\text{O}_2 \rightarrow \text{CH}_3\text{OH}$ reaction in the presence of $Au_xFe_y$ clusters is clearly spontaneous, since the reaction free energy change reaches $\Delta$ $\text{G}_{react} = -2.841 eV$ for $\text{Au}_4\text{Fe}_2$. The primary catalytic effect of $\text{Au}_x\text{Fe}_y$ lies in lowering the radical formation energy from $19.60\text{ eV}$ in the gas phase to only $0.227\text{ eV}$ for the $1:1\text{ Au:Fe}$ composition. Computed reaction barriers for the $\text{Au}_x\text{Fe}_y\text{-}\text{HO}\bullet + \bullet\text{CH}_3$ coupling step range from $1.349$ to $0.269\text{ eV}$, indicating a highly favorable pathway. Among all stoichiometries, $\text{Au}_3\text{Fe}_3$ shows the most pronounced total catalytic enhancement, highlighting the synergistic $\text{Au-Fe}$ role in promoting methane oxidation.
...
#### 3.4 Radical Formation & C-O Coupling
